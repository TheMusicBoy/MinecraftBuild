...
// Every tiered component's vanilla recipe is removed and replaced with a
...
ServerEvents.recipes((event) => {
  ...
  // Per-tier material palette + assembler voltage.
  const TIERS = {
    MV: {
      circ: '#gtceu:circuits/mv', volt: GTValues.MV, board: 'gtceu:plastic_circuit_board',
      plate: 'gtceu:aluminium_plate', wire: 'gtceu:fine_copper_wire',
      cable: 'gtceu:copper_single_cable', dur: 200,
    },
    HV: {
      circ: '#gtceu:circuits/hv', volt: GTValues.HV, board: 'gtceu:epoxy_circuit_board',
      plate: 'gtceu:stainless_steel_plate', wire: 'gtceu:fine_annealed_copper_wire',
      cable: 'gtceu:gold_single_cable', dur: 300,
    },
    EV: {
      circ: '#gtceu:circuits/ev', volt: GTValues.EV, board: 'gtceu:fiber_reinforced_circuit_board',
      plate: 'gtceu:titanium_plate', wire: 'gtceu:fine_aluminium_wire',
      cable: 'gtceu:aluminium_single_cable', dur: 400,
    },
    IV: {
      circ: '#gtceu:circuits/iv', volt: GTValues.IV, board: 'gtceu:multilayer_fiber_reinforced_circuit_board',
      plate: 'gtceu:naquadah_alloy_plate', wire: 'gtceu:fine_platinum_wire',
      cable: 'gtceu:platinum_single_cable', dur: 500,
    },
  };

  // Input counts + solder (mB) + duration multiplier per physical size.
  const SIZE = {
    small:  { b: 1, p: 1, w: 2, c: 1, solder: 72,  durMul: 0.75 },
    medium: { b: 2, p: 2, w: 4, c: 2, solder: 144, durMul: 1.0  },
    large:  { b: 3, p: 4, w: 6, c: 3, solder: 288, durMul: 1.5  },
  };
  ...
  COMPONENTS.forEach(([item, tierKey, size]) => {
    ...
    try {
      ...
      // GregTech Assembler recipe, gated by the tier circuit and voltage.
      event.recipes.gtceu
        ...
        .duration(Math.round(t.dur * s.durMul))
        .EUt(GTValues.VA[t.volt]);
      count++;
    } catch (err) {
  ...
  console.log('[StarT-OC] Gated ' + count + '/' + COMPONENTS.length + ' OpenComputers components behind GregTech tiers (MV/HV/EV/IV).');
});
